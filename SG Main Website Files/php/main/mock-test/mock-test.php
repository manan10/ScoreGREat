...
<html lang="en">
    <head>
        ...
        <title>Score GREat | Mock Test</title>
        <meta charset="utf-8">
    ...
    <body class="bg-light">
        ...
        <div class="container py-4 mx-auto mt-5">
            <form action="eval.php?m-id=<?php if(isset($_GET['m-id'])) echo $_GET['m-id'];?>" onsubmit="return <?php echo $validform;?>;" method="POST">
                <div class="form-group">
                    <label for="que"><b>Question <?php
                        if($_SESSION['q-marked'] == 0)
                    ...
                    <textarea class="form-control mt-1 py-2" name="que" rows="1" id="que" readonly><?php   
                       ...
                       echo "<button type='submit' class='btn my-4 float-right' style='color: white; background-color: black; width: 120px;'><span>Next&nbsp;</span>  <i class='fas fa-angle-right'></i></button>";
                       
                       if($_SESSION['q-marked'] == 0) {
                        echo "<button type='button' id='review' class='btn my-4' style='color: white; background-color: black; width: 120px;'><span>Review&nbsp;</span>  <i class='far fa-check-circle'></i></button>";
                       }
                       else{
                           echo "<button type='button' onclick='set-view.php' id='set-view' class='btn my-4' style='color: white; background-color: black; width: 120px;'><span>Marked Questions&nbsp;</span></button>";
                       }
                    ?>
            </form>
        </div>
    ...
    <script>
        ...
        var refreshId = setInterval(function() {
            // console.log($('.timer').attr('data-seconds-left'));
            if($('.jst-seconds').text()==0 && $('.jst-minutes').text()=="00:"){
                alert("Time's up");
                window.open('review-ques.php?time=0','_self');
                exitInterval();
            }
        }, 1000);
		...
		function checkverbal9(){
			...
			$('#time').val($('.timer').attr('data-seconds-left'));
        ...
        $(document).ready(function(){
            ...
            $('#review').click(function(){
                $.get('review-ques.php',function(){
                    location.reload();
                });
            });
            
            $('#set-view').click(function(){
                window.location.href = "set-view.php";
            });
