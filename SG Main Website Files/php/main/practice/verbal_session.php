<?php
    ...
    if(strcmp($_SESSION['difficulty'],"adaptive")==0){
        ...
        foreach($diffi as $diff){
            $str = "select id from scoregreat.ques_verbal where difficulty='".$diff."';";
        ...
        $_SESSION['nextq'] = $diffiq[mt_rand(0,3)];
    }
    else{
        if(strcmp($_SESSION['difficulty'],"random")==0){
            ...
            $str = "select id from scoregreat.ques_verbal where difficulty='".$_SESSION['difficulty']."'";
        }
        $stmt = $con->prepare($str);
        $stmt->execute();
        $rows = $stmt->fetchAll();
